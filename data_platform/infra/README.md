...
**1. `data_platform_net` not found — stack fails to start**
```
Error: network data_platform_net declared as external, but could not be found
```
Run `docker network create data_platform_net` and retry.

---

**2. Airflow init exits with non-zero code**
Check for missing `AIRFLOW_UID` in `airflow/.env`. On Linux, UID mismatch causes permission errors on mounted volumes. Set `AIRFLOW_UID` to the output of `id -u`.

---

**3. `AIRFLOW__API_AUTH__JWT_SECRET` not set**
Airflow 3.x will refuse to start the API server without this variable. Ensure `airflow/.env` contains a value for `AIRFLOW__API_AUTH__JWT_SECRET`.

---

**4. Nessie fails to start — `Connection refused` to postgres**
Nessie depends on `postgres_container` being reachable on `data_platform_net`. Confirm the postgres stack is up and the container name is exactly `postgres_container`.

---

**5. Spark jobs fail with `S3 access denied` or `NoSuchBucketException`**
- Confirm MinIO is running and the `datalake` bucket exists.
- Check `spark-defaults.conf` credentials (`fs.s3a.access.key`, `fs.s3a.secret.key`).
- The bucket must be created before the first Spark job runs. **TODO:** Document the MinIO bucket bootstrap step.

---

**6. `SparkSubmitOperator` fails — `spark_default` connection not found**
Register the Spark connection in Airflow (see "Add the Spark connection" section above). The connection is not auto-created.

---

**7. dbt fails inside Cosmos — `Connection refused` to spark-thrift:10001**
- Confirm `spark-thrift` container is healthy (`docker ps`).
- Confirm Airflow containers are on `data_platform_net` (check `networks` in the compose file).
- The Thrift container has no explicit healthcheck; wait ~30 s after it appears running before triggering the DAG.

---

**8. Airflow worker cannot reach Spark master — `spark://spark-master:7077` refused**
The Airflow worker container must resolve `spark-master` by hostname. Both must be on `data_platform_net`. If they are in different compose defaults networks, add `data_network` to both (already done in the current compose; verify no manual overrides broke this).

---

**9. Image build fails during JAR download**
JARs are fetched from Maven Central at build time. If the build host has no internet access, pre-download the JARs and `COPY` them into the image, or use a local Nexus/Artifactory mirror. **TODO:** Add offline-build instructions.

---

**10. `pgadmin` container starts but login fails**
Default credentials are `pgadmin4@pgadmin.org` / `admin` (from compose defaults). Override via `PGADMIN_DEFAULT_EMAIL` and `PGADMIN_DEFAULT_PASSWORD` in the root `.env` (which is loaded by the postgres compose via env interpolation).
