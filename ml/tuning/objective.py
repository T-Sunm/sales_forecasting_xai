...
def make_objective(train_path, valid_path):
    ...
    def objective(trial: optuna.Trial):
        # Child run (nested=True) to keep the UI clean
        with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True) as child_run:
            ...
            params = {
                ...
                "n_estimators": 10, # Reduced to 10 for quick testing
            }

            # Log parameters and trial metadata
            ...
            # Log metrics
            mlflow.log_metric("val_rmsle", score)
            mlflow.log_metric("duration_sec", time.time() - start_time)
            
            # Store the run ID in trial's user attributes for tracing back from parent
