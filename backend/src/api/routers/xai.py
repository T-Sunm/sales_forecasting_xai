...
async def get_global_importance(
    ...
    """
    raw_model = manager.get_raw_model()
...
async def get_local_explanation(
    ...
    if request.features:
        ...
        X_sample = store_item_data.iloc[[request.sample_index]]
    else:
    ...
    # Prefer log_units but check logunits for compatibility
    target_col = "log_units" if "log_units" in X_sample.columns else "logunits"
    if target_col in X_sample.columns:
        explanation["actual"] = xai_explainer.safe_float(X_sample.iloc[0][target_col])
