...
def get_historical_weather_average(feature_engineered_data, target_date, store_col, item_col, store_id, item_id):
    """
...
def recursive_forecast(
    ...
    if days_to_forecast <= 1:
        st.info(f"📅 Target date is within historical range. Using direct prediction.")
        return None, None  # Signal to use simple prediction
    
    # Safety check
    if days_to_forecast > max_forecast_days:
        st.warning(f"⚠️ Target date is {days_to_forecast} days away. Limiting recursive forecast to {max_forecast_days} days for performance.")
        days_to_forecast = max_forecast_days
    
    st.info(f"🔄 Performing recursive forecasting for {days_to_forecast} days from {last_historical_date.date()} to {target_date.date()}")
    ...
    for day_offset in range(1, days_to_forecast + 1):
        ...
        # Update progress
        progress_bar.progress(day_offset / days_to_forecast)
